...
(function () {
  ...
  function _wrapWarningsInCard(containerId) {
    ...
    el.innerHTML =
      ...
      + '" onclick="'
      // Close open modals then jump to the Calendar page and scroll to it
      + 'document.querySelectorAll(\'.modal-overlay.open\').forEach(function(m){m.classList.remove(\'open\')});'
      + 'if(typeof showPage===\'function\')showPage(\'calendar\');'
      + 'var cal=document.getElementById(\'calendarSection\');if(cal)setTimeout(function(){cal.scrollIntoView({behavior:\'smooth\'});},50);'
      + '">'
      + 'View Calendar &#8599;'
      + '</button>'
      + '</div>'
